...
namespace RenewalTML.Data
{
    public class AdminActionManager : GenericManager<AdminAction>
    {
        ...
        {
            var list = await _genericRepository.Where(m => m.Type == ("{to:user:" + client.Id + "}")).ToListAsync();
        ...
        public async Task<List<AdminAction>> GetAdminActionForAdmin(Client client, int limit) => await _genericRepository.Where(m => m.AdminId == client.Id).OrderByDescending(m => DateTimeAddon.StringToDateTime(m.Date).Ticks).Take(limit).ToListAsync();
